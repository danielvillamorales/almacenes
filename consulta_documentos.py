...
def ventas_por_fecha(fecha_inicial, fecha_final=datetime.now().strftime("%Y-%m-%d")):
    sql = (
        "select  nvl(upper(prefijo),'Total') , to_char(sum(valor),'$9,999,999,999'), to_char(sum(cantidad),'9,999,999,999')  ,sum(valor) , sum(cantidad) from appalmacenes_viewventasfecha"
...
def presupuesto_almacenes(anio, mes1, mes2):
    sql = (
        " SELECT  to_char(CANTIDAD_AÑO_ANTERIOR,'9,999,999,999'),to_char(NETO_AÑO_ANTERIOR,'$9,999,999,999'), SUCURSAL, NOMBRE_SUCURSAL, CATEGORIA, VENDEDOR, NOMBREVENDEDOR,"
...
def ventasconnombredia(mes, anio):
    sql = """select dia , dia_semana , round(nvl(ecommerce_neto,0)),  
            ...
            round(nvl(tulua_neto,0)),round(nvl(septima_neto,0)),round(nvl(bulevar_neto,0)),round(nvl(estacion_neto,0))
            from (select * from  
            ...
            '61' as outlet,'C6' as obelisco,'UP' as pasto,'79' as outletbog,'CR' as carpa,'WP' as wp,'TU' as tulua,'C7' as septima,'BU' AS bulevar,'GE' as estacion)   
            ))order by dia """
    try:
        # print(sql)
        con = conexion.con()
        ...
        for d in data:
            ...
            totales.bulevar += diaria.bulevar
            totales.estacion += diaria.estacion
...
def presupuesto():
    ...
    if data is not None:
        ...
        porcentaje_promedio_venta = (
            round(valor_venta / valor_presupuesto if valor_presupuesto > 0 else 1, 4)
            * 100
        )
        valor_ano_anterior_str = (
            "{:,}".format(valor_ano_anterior)
            if str(valor_ano_anterior) != "None"
            else 0
        )
        valor_presupuesto_str = (
            "{:,}".format(valor_presupuesto) if str(valor_presupuesto) != "None" else 0
        )
        valor_venta_str = (
            "{:,}".format(valor_venta) if str(valor_venta) != "None" else 0
        )
    # print(ventas_diarias)
    return render_template(
        "presupuesto.html",
        data=data,
        valor_presupuesto=valor_presupuesto_str,
        valor_venta=valor_venta_str,
        valor_ano_anterior=valor_ano_anterior_str,
...
def ventaspordia():
    ...
    totals = datos[1]
    return render_template(
        "ventasdiames.html", data=data, mes=mes, anio=anio, totals=totals
    )
